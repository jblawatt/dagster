...
- `job_config`: The [Job](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.20/#job-v1-batch) itself.

The values for each of these keys is a dictionary with the YAML configuration for the underlying Kubernetes object. Note that while the top-level keys within `dagster-k8s/config` use snake_case notation, the dictionaries within use camelCase, much like they would in the underlying Kubernetes object's YAML.
