...
from .models import backcompat_model_dict, k8s_model_from_dict
...
class UserDefinedDagsterK8sConfig(
    ...
    def __new__(
        ...
        if container_config:
            container_config = backcompat_model_dict(
                kubernetes.client.V1Container, container_config
            )

            # special-case envFrom as well which previously allowed secret_ref instead of secretRef
            # and config_map_ref instead of configMapRef
            if "envFrom" in container_config:
                container_config["envFrom"] = [
                    backcompat_model_dict(kubernetes.client.V1EnvFromSource, env_from)
                    for env_from in container_config["envFrom"]
                ]

        if pod_template_spec_metadata:
            pod_template_spec_metadata = backcompat_model_dict(
                kubernetes.client.V1ObjectMeta, pod_template_spec_metadata
            )

        if pod_spec_config:
            pod_spec_config = backcompat_model_dict(kubernetes.client.V1PodSpec, pod_spec_config)

        if job_config:
            job_config = backcompat_model_dict(kubernetes.client.V1Job, job_config)

        if job_metadata:
            job_metadata = backcompat_model_dict(kubernetes.client.V1ObjectMeta, job_metadata)

        if job_spec_config:
            job_spec_config = backcompat_model_dict(kubernetes.client.V1JobSpec, job_spec_config)
